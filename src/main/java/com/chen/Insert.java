...
//                                System.out.println("组数为"+group_nums+"的段的"+j+"组所占用的块索引列表"+needAddBlockList);
                                int global_block_index=needAddBlockList.get(i);
                                Block needAddBlock=index.getBlock(global_block_index);
//                                System.out.println(j+"组"+"需要添加布隆过滤器的块当前信息");
//                                needAddBlock.printBlockInfo();
                                if(needAddBlock.getUseRowStorage()){//如果当前块是按行存储，则先转换
                                    System.out.println(needAddBlock.getBlockIndex()+"块涉及添加布隆过滤器时的行转列");
                                    needAddBlock.convertToColumnStorage();
                                    operateBlock.add(global_block_index);//还要转换回来的块
                                    needAddBlock.addBF();//当前是按列存的BloomFilter，等待插入
                                }else {
                                    needAddBlock.addBF();
                                }
                            }
...
//                    System.out.println(datasetName+"需要存储的段没有空余位置");
                    ...
                    for(int i=0;i<group_nums;i++){
                        ...
                        operateBlock.add(block_index);//新建的块，都是按列存，转换为按行存
...
//                System.out.println(datasetName+"需要存储的段还未创建");
            ...
            System.out.println("kmer文件路径"+kmerdataPath);
            int num=0;
...
//                        System.out.println("添加kmer时涉及到行存储转列存储");
                        insertblock.convertToColumnStorage();
                        operateBlock.add(global_block_index);
                        num++;
                        insertblock.addElement(insert_bf,kmer);
                    }else {
                        insertblock.addElement(insert_bf,kmer);
                    }
                }
...
//            System.out.println("插入的数据集"+datasetName+"在这个段存储的数据集列表中索引为:"+insertpositon);
            ...
            System.out.println("需要转换回按行存的块"+operateBlock);
            System.out.println("添加kmer时进行的块转换次数："+num);
            //将需要转换回的块转换回行存储
